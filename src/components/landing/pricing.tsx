...
export function Pricing() {
  return (
    <section id="pricing" className="scroll-mt-28 border-t border-rule py-20 md:py-28">
      <div className="section-shell">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lead text-muted">
            Everything happens on Substack. The free edition stands on its own. The paid edition is simply for readers who would like more opportunities, more discoveries, and a little more room to wander.
          </p>
        </div>
          ...
          {PLANS.map((plan) => (
            ...
            >
              ...
              <ul className="mt-8 flex flex-1 flex-col gap-3">
                {plan.perks.map((perk) => (
                  <li key={perk} className="flex gap-3 text-sm leading-snug">
                    <Check
                      className={cn(
              ...
              >
                <a href={SUBSTACK_URL} target="_blank" rel="noreferrer">{plan.cta}</a>
              </Button>
            </article>
          ))}
        </div>
